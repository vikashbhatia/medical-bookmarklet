(async function() {
    ...
    function sV(q, v) {
        ...
        if (e) {
            ...
            e.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: !0 }));
            e.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: !0 }));
    ...
    function gP() {
        let p = document.querySelectorAll('path[d^="M3 17.25"]'), b = [];
        p.forEach(x => { let btn = x.closest('button'); if (btn) b.push(btn); });
        return b;
    }
    ...
    for (let k = 0; k < prows.length; k++) {
        ...
        try {
            let f = gL('Frequency Unit'), du = document.getElementById('frequencyunit'), durU = gL('Duration Unit');
            if (f) {
                let freq = 'DAILY';
                if (/d3|60k|forcan|60000|fluka/i.test(medName)) freq = 'WEEKLY';
                if (/drops/i.test(medName) && /d3/i.test(medName)) freq = 'DAILY';
                sV(f, freq); if (du) sV(du, freq);
        ...
        try {
            let u = document.getElementById('numberof unit ordered'), d = document.getElementById('duration');
            if (u && d) {
    ...
    try {
        ...
        if (Object.keys(patchData).length > 0) {
            await fetch(FB_URL, { method: 'PATCH', body: JSON.stringify(patchData) });
        }
        updateSymptomsField();
    } catch (e) {}
