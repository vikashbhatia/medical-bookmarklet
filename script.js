(async function() {
    const mN = document.getElementById('mobileNbr'),
    ...
    function cTxt(t) {
        ...
        for (let i = 0; i < s.length; i++) {
            if (s[i].innerText && s[i].innerText.includes(t)) {
                (s[i].closest('button') || s[i].closest('a') || s[i]).click();
                return
            }
        }
    }
    ...
    function gL(t) {
        let l = Array.from(document.querySelectorAll('label')).find(x => x.innerText.includes(t));
        return l ? (l.closest('.MuiFormControl-root') || l.parentElement.parentElement).querySelector('input') : null
    }

    function gP() {
        let p = document.querySelectorAll('path[d^="M3 17.25"]'),
            b = [];
        p.forEach(x => {
            let btn = x.closest('button');
            if (btn) b.push(btn)
        });
        return b
    }
    ...
    updateSymptomsField();
    log('📋 1. Mobile & Gender...');
    let g = document.querySelector('input[name="gender"][value="1"]');
    if (g) g.click();
    await w(500);
    log('📞 2. Callback...');
    let imgCb = document.querySelector('img[src*="callback.gif"]');
    if (imgCb)(imgCb.closest('button') || imgCb.closest('a') || imgCb).click();
    await w(500);
    log('👤 3. Checking Name...');
    if (fn && (!fn.value || fn.value.trim() === '')) {
        sV(fn, 'Confirm On Call');
        await w(500)
    }
    await w(300);
    log('💊 4. Rx Info...');
    ...
    for (let k = 0; k < prows.length; k++) {
        ...
        await w(1300);
        let medName = '',
            medInput = document.getElementById('medicineName');
        if (medInput) medName = medInput.value.toLowerCase();
        let isRejected = medName.includes('dizone') || medName.includes('disulfiram') || medName.includes('avil'),
            mnBtn = document.querySelector('input[name="mn"][type="checkbox"]'),
            ntBtn = document.querySelector('input[name="nt"]');
        try {
            if (/meter|eye|inhaler|respules|tears|glim|glip|nasal|rotacaps|spray|ear|powder|met|gp|benadryl|syrup|tel|gluc/i.test(medName)) {
                if (mnBtn && !mnBtn.checked) mnBtn.click();
                if (ntBtn && !ntBtn.checked) ntBtn.click()
            } else if (/shampoo|dsr|ocid|suncros|pan|pantocid|thyronorm|thyrox|omee|spf|d sr/i.test(medName)) {
                if (mnBtn && !mnBtn.checked) mnBtn.click()
            } else {
                if (ntBtn && !ntBtn.checked) ntBtn.click()
            }
        ...
        await w(100);
        try {
            let f = gL('Frequency Unit'),
                du = document.getElementById('frequencyunit'),
                durU = gL('Duration Unit');
            if (f) {
                let freq = 'DAILY';
                if (/d3|60k|forcan|60000|fluka/i.test(medName)) freq = 'WEEKLY';
                if (/drops/i.test(medName) && /d3/i.test(medName)) freq = 'DAILY';
                sV(f, freq);
                if (du) sV(du, freq);
                if (durU) sV(durU, (freq === 'WEEKLY' || freq === 'MONTHLY') ? freq : 'DAYS')
            }
        } catch (e) {}
        await w(100);
        try {
            let u = document.getElementById('numberof unit ordered'),
                d = document.getElementById('duration');
            if (u && d) {
                let val = parseInt(u.value) || 10;
                if (val < 10) val = 10;
                if (mnBtn && mnBtn.checked && ntBtn && ntBtn.checked) val = Math.ceil(val / 2);
                sV(d, val.toString())
            }
        } catch (e) {}
        await w(100);
        try {
            let st = gL('Item Status');
            if (st) sV(st, isRejected ? 'REJECTED' : 'APPROVED')
        } catch (e) {}
        await w(100);
        try {
            let up = Array.from(document.querySelectorAll('button')).find(b => b.innerText.toLowerCase().includes('update'));
            if (up) up.click()
        } catch (e) {}
        await w(1000)
    }
    ...
    try {
        ...
        if (s) {
            ...
            for (let el of nEls) {
                ...
                if (t.includes('Dr.') && !/Vikas Bhatnagar|Prescriber/i.test(t)) {
                    n = t.replace(/^Dr\.\s+Dr\./i, 'Dr.');
                    break
                }
            }
